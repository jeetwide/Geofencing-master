...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
   ...
   /* @NonNull
    ...
    @NonNull
    private Geofence getGeofence() {
        LatLng latLng = Constants.AREA_LANDMARKS.get(Constants.GEOFENCE_ID_Titanium);
        return new Geofence.Builder()
                .setRequestId(Constants.GEOFENCE_ID_Titanium)
                .setExpirationDuration(Geofence.NEVER_EXPIRE)
                .setCircularRegion(latLng.latitude, latLng.longitude, Constants.GEOFENCE_RADIUS_IN_METERS)
                .setNotificationResponsiveness(1000)
                .setTransitionTypes(Geofence.GEOFENCE_TRANSITION_ENTER | Geofence.GEOFENCE_TRANSITION_EXIT)
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        LatLng latLng = Constants.AREA_LANDMARKS.get(Constants.GEOFENCE_ID_Titanium);
        googleMap.addMarker(new MarkerOptions().position(latLng).title("Titanium"));
        googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng, 17f));
/*
        this.googleMap = googleMap;
        LatLng latLng = Constants.AREA_LANDMARKS.get(Constants.GEOFENCE_ID_Ahmedabad);
        googleMap.addMarker(new MarkerOptions().position(latLng).title("Ahmedabad Airport"));
        googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng, 17f));

        this.googleMap = googleMap;
        LatLng latLngone = Constants.AREA_LANDMARKS.get(Constants.GEOFENCE_ID_Hansol_Police);
        googleMap.addMarker(new MarkerOptions().position(latLngone).title("Hansol Police"));
        googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(latLngone, 17f));*/
